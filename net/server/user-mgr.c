              ...
              const char *extra_attrs)
{
    ...
    if (extra_attrs)
        rc = ccnet_db_statement_query (db,
                                       ...
                                       "is_active, extra_attrs, ctime) VALUES (?, ?, ?, ?, ?, ?)",
                                       6, "string", email, "string", password, "int",
                                       is_staff, "int", is_active, "string", extra_attrs, "int64", now);
    else
        rc = ccnet_db_statement_query (db,
                                       "INSERT INTO LDAPUsers (email, password, is_staff, "
                                       "is_active, ctime) VALUES (?, ?, ?, ?, ?)", 5, "string", email,
                                       "string", password, "int", is_staff, "int", is_active, "int64", now);
    if (rc < 0) {
        return rc;
    }
...
static int check_db_table (CcnetDB *db)
{
    ...
    if (db_type == CCNET_DB_TYPE_MYSQL) {
        ...
        sql = "CREATE TABLE IF NOT EXISTS LDAPUsers ("
          ...
          "is_staff BOOL NOT NULL, is_active BOOL NOT NULL, extra_attrs TEXT, ctime INTEGER,"
          "reference_id TEXT)";
        if (ccnet_db_query (db, sql) < 0)
            return -1;
                                   ...
                                   const char *status)
{
    ...
    if (start == -1 && limit == -1) {
        ...
        sql = g_strdup_printf ("SELECT t1.id, t1.email, "
                               ...
                               "ON t1.email = t2.email %s "
                               "WHERE t1.email NOT LIKE '%%@syncwerk_group'",
                               status_condition);
                                      ...
                                      int start, int limit)
{
    ...
    if (start == -1 && limit == -1)
        rc = ccnet_db_statement_foreach_row (db,
                                             ...
                                             "WHERE t1.Email LIKE ? "
                                             "AND t1.email NOT LIKE '%%@syncwerk_group' "
                                             "ORDER BY t1.id",
                                             get_emailusers_cb, &ret,
                                             1, "string", db_patt);
    else
...
ccnet_user_manager_get_superusers(CcnetUserManager *manager)
{
    ...
    snprintf (sql, 512,
              ...
              "LEFT JOIN UserRole t2 "
              "ON t1.email = t2.email "
              "WHERE is_staff = 1 AND t1.email NOT LIKE '%%@syncwerk_group';");
